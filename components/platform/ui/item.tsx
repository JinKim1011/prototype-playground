...
function ItemGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "group/item-group flex w-full flex-col gap-2.5 has-data-[size=sm]:gap-2 has-data-[size=xs]:gap-0",
        className
      )}
      {...props}
    />
...
const itemVariants = cva(
  "group/item flex w-full flex-wrap items-center rounded-none border text-xs transition-colors duration-100 outline-none hover:bg-muted hover:text-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 [a]:transition-colors [a]:hover:bg-muted",
  {
    variants: {
      ...
      size: {
        default: "gap-3.5 px-3 py-2.5",
        sm: "gap-3.5 px-3 py-2.5",
        xs: "gap-3 px-2 py-2 in-data-[slot=dropdown-menu-content]:p-0",
      },
    },
    defaultVariants: {
      variant: "default",
...
function ItemContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-1 gap-1.5 [&+[data-slot=item-content]]:flex-none",
        className
      )}
      {...props}
    />
